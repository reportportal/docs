...
Now you can investigate found defect as default Product Bug/Automation Bug/System Issue/No Defect or custom defects. Both ones will be available to select on Defect Type editor.
...
For example, if you have created custom "Product Bug-1" as sub type of "Product Bugs" group and investigated a few
defects as "Product Bug-1", if you remove "Product Bug-1" from your project, all investigated "Product Bug-1" defects
...
It means that ReportPortal will look for similar fails in the same launch, where **Match issues** was called.
...
And this issue did not appear in previous executions. In order to avoid review and submittion of defect for 200 times
 - call **Match Issue**. ReportPortal will find similar fails in this launch, based on **Auto-Analysis** algorithm.

You can call **Match issues in Launch** only on the manual demand. And it will:

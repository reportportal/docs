...
## Quality Gate Purpose 
 ...
 ReportPortal supports Continuous Testing with build-in functionality - Quality Gates. The primary Quality Gate purpose is to speed up a CI/CD pipeline.
...
The Quality Gate feature is available in the scope of the ReportPortal enterprise version, which is provided for clients in the scope of support paid engagement( min contract amount 50K).
...
The default configuration of ReportPortal doesn't contain Quality Gate. For adding this feature, you need to receive a link to the .jar file from ReportPortal. Download the .jar file and upload it to ReportPortal. Fo that pleases perform, following actions:
...
>**Case 1:** Regression suite has 1000 tests. You want to track that all tests should run every time.
...
In this case, the system will automatically analyze a launch and compare the number of tests in the analyzed launch with a number in the "amount" rule in the Quality Gate. If the number of tests in the launch is less than in the rule, the system fails the rule and Quality Gate.
...
3.  Click on the drop-down: "Add a new rule."
4.  Choose the option "Amount."
5.  Choose option "Tests with an attribute."
6.  Add a number of min allowable amount of tests for the component, feature etc. - N 
7.  Click on the tick 
8.  The rule is added to the Quality Gate

In this case, the system will automatically analyze a launch and compare the number of tests with the specified attribute in the analyzed launch with a number in the "amount" rule in the Quality Gate. If the number of tests is less than in the rule, the system fails the rule and Quality Gate.
...
In this case, on the finish, the system will automatically analyze a launch and compare the failure rate (failed tests/total)of tests with the specified attribute in the analyzed launch with failure rate (failed tests/total) from the rule in the Quality Gate. If the failure rate is more than specified in the rule, the system fails the rule and Quality Gate.

You can add several "Tests with attribute percent" rules to the Quality Gate. But it is impossible to create duplicates.

**Not passing rate in the launch or in a component/feature/etc**

You can use the "Percent rule" in several options: Failure /Not passed.

The failure rule is described in the previous sections.

If you choose the "Not passed" option, the system will calculate the rate as (failed + skipped)/total.
...
>**Case 1:** Regression suite has 1000 tests. You want to track that the run should not have critical issues or Product bugs (or any other)
...
The purpose of the rule is a check and a guarantee that the run does not contain specified issues. But if a launch contains "To investigate", the system can not analyze the system and guarantee that the forbidden issues are absent in a launch.
...
Amount of issues also has 2 options "All tests" and "Tests with an attribute". The purpose of the rule is to limit the number of unwanted defects in the run. With the option "All tests", you can restrict issues for all tests in the launch.
...
In this case, on the finish, the system will automatically analyze a launch and compare failed tests /or failed tests with the specified attribute in the analyzed launch with tests in the baseline. If the system detects a new failure in the launch or tests with specified attributes, it fails a rule. 
...
(?) When you use "latest", the system will use the latest launch with a specified launch name, which has been run before the analyzed launch.
If you want to specify a baseline, you also can add a launch attribute. In this case, the system will use the latest launch with specified launch name and attributes, which have been run before the analyzed launch.
...
Quality Gates plugin can be used as CI/CD tools - in this case, and Quality Gate status will be sent to CI/CD pipeline.
But also, Quality Gates can be used just as a way of test results analysis.
...
If the Jenkins received a response about QualityGate status from RP, the build status should be adequately marked:

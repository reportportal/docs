...
If you want to reduce time costs, you can deploy the  ReportPortal with a service Analyzer. For that you should add an info about this service in a docker-compose file and deploy service Analyzer. Service ElasticSearch will be running together with Analyzer.
...
Service Analyzer forms a query with following data to the  service ElasticSearch.:
* LogMessage;
* Launch ID;
* UniqueID test item; 
* Defect type;

So each log with its defect type is saved in ElasticSearch as a separate document. All document creates Index. So the more test results you analyze , the more data in the ElasticSearch appears, the more accurate results you will get in the end of the process. 
...
Test items of a launch in Debug mode are not sent to service Analyzer. If you delete test item or move it to the Debug, the test item is removed from a base.

...
An auto-analyzer is presented by a combination of two services: ElasticSearch and Analyzer service. 
...
Service Analyzer forms a query with log data to the ElasticSearch.The query is contained following info:
* LogMessage;
* Launch ID;
* UniqueID test item; 
...
Parameters **MinDocFreq** and **MinTermFreque** are involved in the calculation of TF-IDF coefficient. So you can normalize log and reduce the importance of frequent but useless words in a log (f.e. "Java" or prepositions...) manually. 

...
:::note
If you have a pre-created Internal user, you can't log in by SAML using their
...
However, you can't provision user from SAML Provider to ReportPortal if you
...
`john_weak`.
In oposite case, the user will not be able to login to ReportPortal.
You can fix it by changing the login name or email or delete the user from
the ReportPortal.

## SAML provider requirements

:::tip
There are detailed manuals for configuration of
...
<MediaViewer src={require('./img/SAMLPlugin.png')} alt="SAML plugin configuration to integrate with our qa automation dashboard" />

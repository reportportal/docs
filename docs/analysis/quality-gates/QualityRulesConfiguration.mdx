...
description: Configure Quality Rules for automated quality control.
---
...
:::info
Admin, Manager, and user with edit permissions can configure Quality Gate rules.
:::
...
When two or more Quality Gates are created, you can change their order by moving a Quality Gate up or down using drag-and-drop to adjust its priority. The higher a Quality Gate appears in the list, the higher its priority.

<MediaViewer src="https://youtu.be/cygIFYewbas" alt="Drag-n-drop Quality Gate in our test results dashboard" type="video" />
...
For defining test uniqueness, our continuous testing platform uses [Test Case ID principles](../../working-with-test-results/TestCaseId.md).

...
A user is on All launches, he clicks on "To investigate" and opens a list with items. When a user clicks on a pencil next to a defect type, the system opens the ["Make decision" modal](./ManualAnalysis.mdx#make-decision-modal-redesign). In this modal a user can see all items with "To investigate" defect type and the same failure reason.

<MediaViewer src="https://youtu.be/FyDdMX_PVyc" alt="Search for the similar To investigate items" type="video" />

...
<MediaViewer src={require('./img/project-settings/ProjectSettings1.png')} alt="How to find project settings in ReportPortal" />

To open the "Project Settings" page, click the corresponding icon in the left menu.

<MediaViewer src={require('./img/project-settings/ProjectSettings2.png')} alt="Project settings in the left menu" />

:::info
All users can view the project settings.<br />
Admin, Manager, and user with edit permissions can update project settings.
:::

The project settings are divided into following tabs: "General", "Integrations", "Notifications", "Defect Types", Log Types, "Analyzer", "Pattern-Analysis", "Demo Data" and "Quality Gates".
...
The first one is API service back-end environment variables that are described in [ReportPortal jobs configuration](../../installation-and-deployment/advanced-installation/JobsConfiguration.md).

The second one is the "General" tab you can configure parameter of data storage in ReportPortal for this project:

- **Name** – is a disabled field, can be edited on Organization level -> [Projects page](../organization-level/Projects.mdx).

In addition to the Project Settings General section, you can find the project name on the left menu and in the URL.

<MediaViewer src={require('./img/project-settings/ProjectSettings3.png')} alt="Where can I find project name in ReportPortal" />
...
:::important
Project settings cannot exceed organizational settings.
:::

## Integrations

Our test reporting tool integrates seamlessly, allowing for a streamlined connection with external systems.

<MediaViewer src={require('./img/project-settings/ProjectSettings4.png')} alt="Integrations tab in the Project Settings" />

Integrations can be added at the [instance level](../instance-level/Plugins.md), making them applicable to all projects, or they can be configured specifically for a project in the Project Settings.

For those who require a different configuration from other projects, or who want to integrate only their specific project with an external system, follow these steps:

1. Go to the Project settings -> Integrations tab.
2. Click on one of the shown integration panels.
3. Click on the "Add Project Integration" button.

Performing these steps will unlink your current project from the global (instance level) settings and initiate a project-specific configuration.

<MediaViewer src={require('./img/project-settings/ProjectSettings5.png')} alt="Add Project Integration button" />

## Notifications

The *Notifications* tab provides the ability to activate notifications for the project through Email, Slack, Telegram, or Teams.

ReportPortal provides a way to add multiple rules with specific conditions. The notification rule should be unique per Notification group – Email, Slack, Telegram, Teams. For example: In case, Launch names, Attributes.

:::info
Admin, Manager, and user with edit permissions can configure notifications.
:::

### Email notifications

:::tip
For using this functionality be sure that the Email server is configured on the instance or on the project level.
:::
...
<MediaViewer src={require('./img/project-settings/ProjectSettings6.png')} alt="Create Email notification rule form" />

**Rule name** – from 1 to 55 characters, mandatory field

**Email** – optional field

Required unless the Launch Owner is selected as the recipient via the checkbox.
...
**In case** – dropdown field; by default, the value is set to *Always*:
...
:::tip
To use this functionality, ensure that the Slack plugin is uploaded and enabled. For that, go to the instance level -> [Plugins page](../instance-level/Plugins.md).
:::
...
<MediaViewer src={require('./img/project-settings/ProjectSettings7.png')} alt="Create Slack notification rule form" />

**Rule name** – from 1 to 55 characters, mandatory field

**Webhook URL** – mandatory field (it is configured on the [Slack side](../../integrations/notifications/Slack.mdx#configuration-on-slack-side)
...
:::tip
To use this functionality, ensure that the Telegram plugin is uploaded and enabled. For that, go to the instance level -> [Plugins page](../instance-level/Plugins.md).
:::
...
<MediaViewer src={require('./img/project-settings/ProjectSettings8.png')} alt="Create Telegram notification rule form" />

**Rule name** – from 1 to 55 characters, mandatory field

**Chat id** – mandatory field, copied from group info in [Telegram](../../integrations/notifications/Telegram.mdx#configuration-on-telegram-side)

:::important
Add ‘-’ before the chat id.
:::

**Bot Api Key** - mandatory field, received during registration [your Telegram bot](../../integrations/notifications/Telegram.mdx#configuration-on-telegram-side)
...
1. Go to the Project settings -> "Defect Types" tab.
2. Click "Create Defect Type" button at the top of the page or "Create a new Defect Type" button under any default defect type.

<MediaViewer src={require('./img/project-settings/ProjectSettings9.png')} alt="Create Defect Type" />

3. Enter "Defect name" and "Abbreviation", select color for defect presentation on the launches grid and widgets.
4. Click "Create" button.

<MediaViewer src={require('./img/project-settings/ProjectSettings10.png')} alt="Select color for defect type" />

Add more custom defects, if you need it. The system allows us to add up to ten subtypes of each default defect type.

Now you can do bug triage and set defect types as default Product Bug/Automation Bug/System Issue/No Defect/TO investigate or custom defects. All of them will be available to select on the Defect Type editor. For more information refer to the [Investigation of failure](../../working-with-test-results/InvestigationOfFailure.mdx) topic.
...
:::
:::tip
If you have deleted a custom type from the system, all test executions with this deleted defect type get the parent type of the group in which deleted defect has belonged. Statistics updates in all launches where a deleted defect was present.<br />
...
:::info
Admin, Manager, and user with edit permissions can configure log types.
:::
...
<MediaViewer src={require('./img/project-settings/CreateLogType.png')} alt="Create Log Type in our test report dashboard" />
...
<MediaViewer src={require('./img/project-settings/FilterOff.png')} alt="Show in log filter flag for just created log type" />
...
<MediaViewer src={require('./img/project-settings/EditCustom.png')} alt="Edit custom log type" />

:::note
Log type name and Log level of a system log type cannot be changed.
:::

<MediaViewer src={require('./img/project-settings/EditSystem.png')} alt="Edit system log type" />
...
<MediaViewer src={require('./img/project-settings/ShowInLogFilter.png')} alt="Managing Show in log filter" />
...
<MediaViewer src={require('./img/project-settings/Delete.png')} alt="Delete custom log type" />
...
For more information on this topic, please refer to [Auto-Analysis documentation](../../analysis/AutoAnalysisOfLaunches.mdx).

## Pattern Analysis

The "Pattern Analysis" feature allows identifying recurring failure patterns and common behaviors across test results.

For more details, see the [Pattern Analysis documentation](../../analysis/PatternAnalysis.mdx).
...
Quality Gates allow defining business-friendly quality rules and acceptance criteria for launches, helping QA teams and stakeholders evaluate release readiness based on testing results.

Explore the full capabilities of [Quality Gates](../../analysis/quality-gates/index.md).

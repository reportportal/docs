...
You can also track the same metrics in ReportPortal with [dashboards and widgets](../../../dashboards-and-visualization/).
...
[Quality Gates](../../../analysis/quality-gates/) evaluate test launches against configured rules: pass rate, number of defects by type, test count, and new failures compared to baselines. Teams set criteria like "block release if more than 5 Product Bugs are found" or "pass rate must be > 95%." Quality Gates integrate with CI/CD pipelines to enforce consistent, automated release decisions.

...
ReportPortal leverages AI assistants via [MCP](../../integrations/ai-agents/MCPServer.mdx) to accelerate testing at every stage of the lifecycle. The ReportPortal MCP Server connects AI clients such as Claude, Cursor, and GitHub Copilot using the Model Context Protocol (MCP). These assistants can help teams create test cases, organize plans, and review reported results.

## Design: AI-assisted test case creation

AI assistants connected via the ReportPortal MCP Server can generate, review, and refine test cases directly in the [Test Management System](../../test-management-system/index.md). Instead of writing every scenario from scratch, QA teams can prompt AI to create [test cases](../../test-management-system/test-case-library/TestCases.md) based on requirements — then review and refine them in ReportPortal.
...
MCP tools enable AI to create and manage [test folders](../../test-management-system/test-case-library/Folders.md), [milestones](../../test-management-system/milestones/index.md), and [test plans](../../test-management-system/milestones/TestPlan.md). AI can help structure test work efficiently, while teams keep control over strategy and prioritization.
...
ReportPortal [consolidates results](./UnifiedTestReporting.md) from automated tests, [agentic QA tests](../../test-results-reporting/AgenticTestResults.md), and manual testing in one place. All test execution data — [logs, screenshots, and other artifacts](./RichArtifactsInTestReports.md) — is available for analysis and investigation. AI assistants can access that context through MCP.
...
ReportPortal's [Unique Error Analysis](../../analysis/UniqueErrorAnalysis.mdx) and [Auto-Analysis](../../analysis/AutoAnalysisOfLaunches.mdx) automatically group related failures and suggest root causes. Instead of manually reviewing hundreds of logs, AI surfaces the signal: 20 failures might be grouped into 3 unique errors. [ML suggestions](../../analysis/MLSuggestions.md) can auto-classify defect types based on historical patterns. For an overview of this built-in layer, see [AI-based failure reason detection](./AIFailureReasonDetection.md).
...
[Quality Gates](../../analysis/quality-gates/index.md) evaluate test launches against configured rules: pass rate, number of defects by type, test count, and new failures compared to baselines. Teams set criteria like "block release if more than 5 Product Bugs are found" or "pass rate must be > 95%." Quality Gates integrate with CI/CD pipelines to enforce consistent, automated release decisions.
...
To connect an assistant, start with [MCP Server](../../integrations/ai-agents/MCPServer.mdx). For cases and plans, see [Test Management System](../../test-management-system/index.md). To report agent sessions, see [Agentic test results](../../test-results-reporting/AgenticTestResults.md).

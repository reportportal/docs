...
description: Manage launch operations including editing attributes, merging launches, and bulk actions to streamline test execution reporting workflows.
---
...
:::note
Launches cannot be merged if the selected launches contain a mix of Agentic and Automation launch types.
:::
...
:::important
No fixed limit, but merging ~20 launches at a time is recommended for stable performance.
:::
...
You can export the launch report in the following formats: PDF, XLS, HTML. This feature supports test results download as Excel (XLS), PDF, HTML, providing flexibility for further analysis or sharing.
...
:::info
Export launch report with nested steps attachments is available starting from version **26.0.2**.<br />
Attachments from nested steps are organized according to their hierarchy within the test structure. Parent step attachments are placed directly in the corresponding Step folder.<br />
Each nested step has its own dedicated subfolder (one folder per nested step), which contains only that nested step’s attachments.<br />
This structure ensures that attachments from nested steps are clearly separated and easy to locate within the exported archive.
:::

<MediaViewer src={require('./img/operations-under-launches/NestedStepsAttachments.png')} alt="Attachment names" />

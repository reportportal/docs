...
description: Review test logs by level in ReportPortal. Filter Trace to Fatal messages, inspect screenshots, and debug failed test steps faster.
---
...
When "Colorized background" is enabled, all logs receive background colors according to the configuration defined in [Project settings -> Log types](../configuration/project-level/ProjectSettings.mdx#log-types). Log entries will display the appropriate label color, background color, text color, and text style (bold or normal), making it easier to distinguish log types visually.

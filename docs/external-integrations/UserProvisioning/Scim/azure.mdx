...
Choose `Integrate any other application you don't find in the gallery (Non-gallery)` bullet
and click `Create` button.

<MediaViewer src={require('../../img/scimAzure/3.png')} alt="Fill application fields" />

5. After the creation of an application, go to the `Provisioning` section and click `New configuration`.
...
:::note
`?aadOptscim062020` is a special parameter that allows you to use the SCIM compatible protocol.
...
2. In the `Attribute Mappings` find and delete the `urn:ietf:params:scim:schemas:extension:enterprise:2.0:User:manager` attribute.
For some reasons Azure sends this attribute with the wrong SCIM format. This attribute is not used in ReportPortal.
...
For the correct synchronization of the admin role, you need to create a new `App-role` in Azure.
...
8. Select the `Show advanced options` checkbox in the bottom and click to `Edit attribute list for customappsso` link.

<MediaViewer src={require('../../img/scimAzure/14.png')} alt="Advanced options" />

9. You see a `Edit Attribute List`. In the bottom of the list fill empty fields with the name `roles` and type `String`.
...
After these steps, then ypu add users or groups to the Azure project you can select

...
Use ReportPortal [integration guides](../integrations/ImplementOwnIntegration.md) and [reporting SDKs](./ReportingSDK.md) for various programming languages to send results from your agent pipeline via the ReportPortal API.
...
To [distinguish](../working-with-test-results/ViewLaunches.mdx#launches-tab) agentic launches from classic automation in the ReportPortal UI, include the following system attribute when starting a launch:

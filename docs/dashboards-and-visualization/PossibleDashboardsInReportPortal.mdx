...
You can configure:
[**Passing rate widget**](./PassingRateSummary.mdx) that shows a passing rate for a latest launch "API suite'

[**Most popular pattern**](./MostPopularPatternTableTop20.mdx) tracks TOP-20 problems in the last and previous runs of this suite.

:::note
For Most popular pattern table, you should you create a set of rules and run Pattern Analysis
:::

With [**Investigated percentage of launches**](./InvestigatedPercentageOfLaunches.mdx) you can find out the status of failure investigations. You will be able to evaluate team performance and consistency of results.

[**Failed cases trend chart**](./FailedCasesTrendChart.mdx) shows the history of failures in previous runs.

[**Duration chart**](./LaunchesDurationChart.mdx) will be very helpful for those who track duration KPI and want to increase the speed of tests run.

[**Test growth trend chart**](./TestCasesGrowthTrendChart.mdx) shows you the speed of new test cases creation.

Also, you can create [**"Most flaky test cases"**](./FlakyTestCasesTableTop50.mdx) and [**"Most failed test case"**](./MostFailedTestCasesTableTop50.mdx) and find the most unstable items which should be taken into account.

Let's assume that you have a lot of test results and a lot of teams.

You can create [**Overall statistics**](./OverallStatistics.mdx) and [**Launches table**](./LaunchesTable.mdx), and now a team who is responsible for API suite has no need to go to the test results. It can use only this dashboard which gives enough information for test failure management.
...
- With [**Cumulative trend chart**](./CumulativeTrendChart.mdx) you will be able to compare different versions on one chart, to compare different runs for the current regression
- [**Component Health Check Widget**](./ComponentHealthCheck.mdx) would show you product:

* on different env
* on a different dimension
* by business metrics / by features/ by user roles / by etc

Also with a help of [**Component Health Check Widget**](./ComponentHealthCheck.mdx) you can create a Test Pyramid.

:::note
You need to report test executions with attributes which specified needed metrics or envs
:::

- [**Unique bugs table**](./UniqueBugsTable.mdx) helps you collect and analyze new bugs in the system

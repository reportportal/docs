...
Email Server plugin is available in ReportPortal on the Instance plugins page.

You don't need to download. It is already available in ReportPortal on the Instance plugins page.
...
:::info
Admin, Manager, and user with edit permissions can create/edit/delete Email Server integration.<br />
...
3. Click on "Email Server" panel.
4. Click on "Add integration" button.

<MediaViewer src={require('./img/email-server/EmailGlobal1.png')} alt="Add Global Email Server integration in ReportPortal" />
...
`Authorization`: OFF/ON (radio button, OFF by default)
`Username`: <string> (shown and required when Authorization is ON)
`Password`: <user email password> (shown and required when Authorization is ON)
`Encryption`: None/TLS/SSL (radio button, required)
```
...
<MediaViewer src={require('./img/email-server/EmailGlobal2.png')} alt="Create Global Email Server integration form" />

After Email Server integration is added, the configuration will apply to all projects within the instance.

## Organizational Email Server integration

To configure the SMTP server for the whole organization:

1. Log in to ReportPortal as Admin or Manager.
2. Go to Organization settings -> Integrations tab.
3. Click on "Email Server" panel.
4. Click on "Create Organizational Integration" button. If a Global Email Server integration already exists, click "Create Integration" instead.

<MediaViewer src={require('./img/email-server/EmailOrganizational1.png')} alt="Create Organizational Email Server integration in our test automation metrics dashboard" />
...
<MediaViewer src={require('./img/email-server/EmailOrganizational2.png')} alt="Create Organizational Email Server integration form" />

After Organizational Email Server integration is added, the configuration will apply to all projects within the organization that do not have an individual Email Server integration.

:::note
Only one Organizational Email Server integration can be added per organization. If a Global Email Server integration exists, it becomes inactive after the organizational integration is created.
:::

:::info
Admin and Manager can create an Organizational Email Server integration.<br />
Member can only view the list of available integrations.
:::
...
3. Click on "Email Server" panel.
4. Click on "Create Project Integration" button.

<MediaViewer src={require('./img/email-server/EmailProject1.png')} alt="Create Project Email Server integration in ReportPortal" />
...
<MediaViewer src={require('./img/email-server/EmailProject2.png')} alt="Create Project Email Server integration form" />

<MediaViewer src="https://youtu.be/OVnc5nxwp24" alt="Email Server integration" type="video" />

## Create Email notification rule

1. Go to the Project settings -> [Notifications](../../configuration/project-level/ProjectSettings.mdx#email-notifications).

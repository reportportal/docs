...
description: Enable secure single sign-on (SSO) in ReportPortal by integrating with Okta SAML, streamlining user authentication and enhancing access management.
---
...
:::tip
Before starting, please read about the [SAML provider](../Overview).
:::
...
More details about the ACS URL you can find in the [SAML Overview](../Overview#identity-provider-configuration).
...
about the identifier you can find in the [SAML Overview](../Overview#identifier).

In the `Name ID format` field, select `EmailAddress`. You can use the UNSPECIFIED format as well.
More information about the Name ID format you can find in the [SAML Overview](../Overview#reportportal-configuration).
...
login. Read more about [JIT provisioning](../Overview#user-jit-provisioning)
:::
...
[SAML Overview](../Overview#reportportal-configuration)

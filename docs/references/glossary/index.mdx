...
<MediaViewer src={require('./img/Glossary10.png')} alt="Example of Nested step in our centralized test automation tool" />
...
A key–value label that can be attached to launches, suites, tests, and steps. Attributes help categorize and group test data, filter results, and build widgets based on those filters. They can be reported via agents or API, and also added or edited in the UI.

<MediaViewer src={require('./img/Attributes.png')} alt="Attributes in our test results dashboard" />

## Tag

A simple label used in the Test Case Library to categorize and find test cases. Tags are created in the UI and stored as attributes with the key tag (for example, `tag: smoke`).

<MediaViewer src={require('./img/Tags.png')} alt="Tags in our test report dashboard" />

## Test Case ID In Automation
...
## Test Case ID In Test Case Library

A unique identifier automatically assigned to each test case within a project in the Test Case Library. It follows the format TC + sequential number (for example, TC65) and is used to reference test cases in the Test Management System.

<MediaViewer src={require('./img/TMSTestCaseID.png')} alt="Test Case ID in the Test Case Library" />

## Milestone

A planning unit in the Test Management System that defines the time frame and context for testing — for example, a release, sprint, feature, or another delivery goal. Within a milestone, teams organize Test Plans, track coverage and deadlines, and prepare test cases for manual launches.

<MediaViewer src={require('./img/Milestones.png')} alt="Planning in the Test Management System" />
...
Organizations are a scalable way to group and manage multiple projects in ReportPortal. Designed primarily for enterprise teams, they introduce a two-level hierarchy that allows you to organize testing efforts by teams, test types, or business units. With Organizations, you can apply shared settings across projects, streamline permissions, and reduce admin work when managing large testing structures. Each organization contains one or more projects, making it easier to keep everything clear, consistent, and under control – especially when your team is growing or your testing needs become more complex.

...
To run your tests in any kind of CI, you firstly need a way to execute them in
...
`pytest` command. Your tests should be already integrated with an ReportPortal agent. To
...
We are going to use `.gitlab-ci.yml` file to store and modify our CI pipeline,
...
find in official [GitLab documentation](https://docs.gitlab.com/ee/ci/yaml/gitlab_ci_yaml.html).

## Define pipeline structure and rules

Before running tests, we need to define steps we want to pass in our pipeline.
This is the common thing for any language or framework you might use, so let’s
do that.

First, we need to securely get our test credentials from a secured storage.
These credentials should not be stored in any kind of logs, artifacts,
attachments during pipeline execution to avoid security britches. Second, we
...
get more control on application-under-test quality, this will be described in
the last section of the current article. Additionally, we will disable on-push
pipeline runs and allow only manual triggering.
...
Notice we put `needs: ['secrets']` property to run test job only after we set
up secrets.
...
As prerequisites configure your Vault server for using with GitLab as described
...
can definitely install it in `before_script` section, it's better to use
`vault:latest` base image to just get it. Instead in `before_script` we will
install just a `curl` tool to be able to call GitLab API.

This how Secrets section will look like:
```yaml
secrets:
  stage: secrets
  image: vault:latest
...
them as project variables to use further in pipeline. To configure Vault CLI
for using necessary server we can use these environment variables:
...
So, our Secrets section in pipeline file will look like that:
```yaml
secrets:
  stage: secrets
  image: vault:latest
...
Where `tests-develop` in `VAULT_TOKEN` variable initialization is a role name
you should configure on Vault server as prerequisites (with
`vault policy write [role name]` command). `CI_JOB_JWT` is an environment
...
`secrets/demo/test` path on Vault and named `rp.admin.password` and
`rp.demo.key` respectively.

To set pipeline variables, we need a GitLab API token. Create it with the
following steps:
1. Go to GitLab user's profile page.
2. In the left menu select Access Tokens.
3. Enter Token name, E.G.: `gitlab_cicd`.
4. Select Expiration date of the token.
5. Check “api” in Select scopes section.
...
2. Click Expand on Variables section.
3. Click Add variable.
4. Fill Key and Value fields.
5. (Strongly recommended) check Protect variable and Mask variable checkboxes.

For our example we need 3 such variable:
* GITLAB_API_TOKEN – for storing Access token we created, to use GitLab API.
* RP_ADMIN_PASSWORD – example secret.
* RP_DEMO_KEY – another example secret.

To finish up our secret setup, we need to update corresponding section once
again:
```yaml
secrets:
  stage: secrets
...
In two new script commands we call GitLab API with `curl` command setting up
...
environment variable to refer our current project. Notice that you need to
...
sake of security, to avoid accidental reveal of our secrets in case of request
...
As for secrets we need to start with defining base worker image we are going to
use. Unfortunately, there is no official Kotlin Docker images on Docker Hub, so
we need to install it ourselves, in `before_script` section. But Kotlin is a
...
Here is updated "test" section of out pipeline Yaml with Kotlin installation:
```yaml
test:
  stage: test
  needs: ['secrets']
...
As you can see, we install `unzip`, then download necessary Kotlin version with
curl, unzip it to `/opt` and update `PATH` variable to make Kotlin compiler
available from any folder in command line. Kotlin version was moved to a
variable to ease further pipeline update.
...
replacement, but a reference to a variable.

### Python tests

Python tests is rather easy to configure, comparing to Kotlin. We start with
defining base worker image and we are going to use `python:3.10.12`. This is
Debian-based image, so we can also install additional software with `apt-get`
if necessary. In `before_script` section we can install our requirements for
the project and run tests in the main section with `pytest` command.

Here is updated "test" section of out pipeline Yaml:
```yaml
test:
  stage: test
  needs: ['secrets']
...
As you can see, we pass `RP_DEMO_KEY` through command line, that’s safe, since
this is not replacement, but a reference to a variable. You will see just a
...
control on application-under-test quality. To enable and configure Quality Gates plugin on
ReportPortal, please, refer [corresponding documentation](/category/quality-gates).

ReportPortal Quality Gates plugin doesn't have native integration with GitLab, so we
need to use custom approach here. One way is polling ReportPortal Launch info API for
specific value, which Quality Gates plugin leave after passing. For that we need Launch
UUID, which is unique Launch identifier.

### Getting Launch UUID for Kotlin tests

Every Java Agent has specific properties which control Launch UUID printing,
since July 2023. To ensure in that check your Agent has
`com.epam.reportportal:client-java` library dependency of version 5.1.21 or
late. It’s generally safe to add exclusion and put newer version of the library
...
`rp.launch.uuid.print = true`. It will output a line like this into console:
...
usually hides test output streams and just print truncated stack trace in case
...
Here is updated "test" stage which does these things:
```yaml
test:
  stage: test
  needs: ['secrets']
...
* We used `--console=plain` Gradle parameter to make output suitable for saving in a file.
* To preserve console output, we used `tee` command, which copies standard input to each specified file, and to standard output.
* We used `sed` command to format and save our Launch UUID into `launch.env` file.
* Thus, we got preformatted `launch.env` file and attached as an artifact, which then will be used to populate environment variables by GitLab.
...
tests, since we are going to decide about test status on Quality Gates step. This can be
done by adding `allow_failure: true` field:
```yaml
test:
  stage: test
  needs: ['secrets']
...
As the next step we need to add `quality-gate` stage to our pipeline:
```yaml
stages:
  - secrets
  - test
...
Notice we put `needs: ['test']` property to run Quality Gates job only after we pass
tests.

Third step will be a little bit tricky. Since we don't specify any image, we
...
have `curl` and `jq` tools to call ReportPortal API, so we need install them in
`before_script` section. Let's make it universal for Debian and Alpine like
that:
```yaml
quality-gate:
  stage: quality-gate
...
packaging tool, in case `apt-get` is not exists and therefore returning failure
result.

Now, it's time to finish our script, this will be rather complex one:
```yaml
quality-gate:
  stage: quality-gate
  needs: ['test']
...
Notice, we moved ReportPortal base URL configuration, polling timeout and request timeout
to variables section to ease their update. In script we are polling our ReportPortal
instance for Launch info with `curl` and Launch UUID, which we got in previous
step. After that with `jq` we are trying to read specific field in response
JSON or return empty value if Quality Gate is not passed yet, or request is failed. In
case of empty response, we retry our call to ReportPortal. Finally, we compare `jq`
...
ReportPortal does not have native integration with GitLab, but that’s not something which
...
And here are corresponding pipeline files, which we implemented:

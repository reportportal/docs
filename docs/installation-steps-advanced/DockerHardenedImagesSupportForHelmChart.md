---
sidebar_position: 8
sidebar_label: Docker Hardened Images support for Helm Chart
description: Docker Hardened Images for ReportPortal Helm. Configure PostgreSQL and RabbitMQ with DHI registry, security contexts, and Helm values.
---

# Docker Hardened Images support for Helm Chart
...
:::note
DHI RabbitMQ uses `rabbitmq-server` as the entrypoint (not `docker-entrypoint.sh`). Prefer runtime tags from the catalog; avoid assuming Docker Official `-management` tag names.
:::
...
- [Parameters reference](https://github.com/reportportal/kubernetes/blob/develop/docs/parameters-reference.md)
